...
- (void)layoutSubviews{
    ...
    if (_needsReload) {
        ...
        {
            offset = 0;
            _scrollView.scrollEnabled = (_pageCount == 2);
        }
        //changed by clei
        float count = (_pageCount < 3) ? _pageCount : 3;
        //----
        switch (_orientation)
        {
            case FlowViewOrientationHorizontal:
                _scrollView.frame = CGRectMake(0, 0, _pageSize.width, _pageSize.height);
                _scrollView.contentSize = CGSizeMake(_pageSize.width * count,_pageSize.height);
            ...
            {
                _scrollView.frame = CGRectMake(0, 0, _pageSize.width, _pageSize.height);
                _scrollView.contentSize = CGSizeMake(_pageSize.width ,_pageSize.height * count);
                CGPoint theCenter = CGPointMake(CGRectGetMidX(self.bounds), CGRectGetMidY(self.bounds));
                _scrollView.center = theCenter;
...
- (void)initVisibleCellAppearance
{
    ...
    _visibleRange = (_pageCount < 3) ? NSMakeRange(0, _pageCount) : NSMakeRange(0, 3);
    //---
    NSInteger  index = 0;
    index = (_currentPageIndex == 0) ? _pageCount - 1 : _currentPageIndex - 1;
    //add by clei
    index = (_pageCount < 3) ? _currentPageIndex : index;
    //-----
    ...
    index = (_pageCount < 3) ? _currentPageIndex + 1 : _currentPageIndex;
    //---
...
- (void)initReuseCell
{
    if (_pageCount == 2)
    {
        ...
        UIView  *cell2 =  [_dataSource flowView:self cellForPageAtIndex:index];
        cell2.tag = index + kTagOffset;
        cell2.frame = CGRectMake(_pageSize.width * index, 0, _pageSize.width, _pageSize.height);
        if (cell2)
        {
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)aScrollView {
    ...
    if (_pageCount < 3)
    {
        _currentPageIndex = x /(_pageSize.width * _minimumPageScale);
        for (int index = 0; index < _reusableCells.count; index++)
        {
            UIView *reuseCell = _reusableCells[index];
            reuseCell.tag = index + kTagOffset;
            [_delegate didReloadData:reuseCell cellForPageAtIndex:index];
        }
        return;
